defmodule XmlToKeywordTest do
  ...
  test "Convert a structure a little more complex without attributes on parent node" do
    xml = XmlToKeyword.convert("<body attr='value'><foo><do><redo>bar</redo></do></foo></body>")
    assert xml == {:ok, {:body, [:body, %{attr: "value"}, [foo: [do: [redo: "bar"]]]]}}
  end
